...
const displayCategories = categories => {
    const newsCategoriList = document.getElementById('newsCategories');
    categories.forEach(category => {
        const categoryList = document.createElement('li');
        categoryList.innerHTML = `
        <a onclick="fatchCatagoryNews('${category.category_id}', '${category.category_name}'); makeBtnActive('${category.category_id}', '${category.category_name}')" id="${category.category_id}" class="categoryBtn hover:text-indigo-600" href="#">${category.category_name}</a>
        `;
        newsCategoriList.appendChild(categoryList);
...
// display news
const dsiplayNews = (news, categoryName, sortField) => {
    const newsContainer = document.getElementById('newsContainer');
    const categoryNewsInfo = document.getElementById('categoryNewsInfo');
    newsContainer.textContent = '';
    if (news.status) {
        categoryNewsInfo.innerText = `${news.data.length} Items Found For Category ${categoryName}`;
        ...
        categoryNewsInfo.innerText = `No Items Found For Category ${categoryName}`;
    ...
    document.getElementById('todayPiBtn').addEventListener('click', function () {
        ...
        if (filterdData.length == 0) {
            ...
            newsContainer.innerHTML = `<section class="container mx-auto">
            <div class=" bg-white p-7">
                <p class="text-center text-2xl text-black font-medium">No News Found</p>
            </div>
        </section>`
            toggleSpinner(false);
        }
        else {
            catagoryNewsInfo.innerText = `${filterdData.length} Items Found For Category ${categoryName}`;
            newsContainer.innerHTML = `<section class="container mx-auto">
            <div class=" bg-white p-7">
                <p class="text-center text-2xl text-black font-medium">No News Found</p>
            </div>
    ...
    document.getElementById('trendingBtn').addEventListener('click', function () {
        ...
        if (filterdData.length == 0) {
            ...
            categoryNewsInfo.innerText = `${filterdData.length} Items Found For Category ${categoryName}`;
            newsContainer.innerHTML = `<section class="container mx-auto">
            <div class=" bg-white p-7">
                <p class="text-center text-2xl text-black font-medium">No News Found</p>
            </div>
